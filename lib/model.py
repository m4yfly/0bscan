...
from config import NetConfig, GlobalConfig
import time
import traceback
...
class SiteJob(object):
    ...
    def change_state(self):
        ...
        if self.job_state == JobState.cms_detect and self.cms_task_que.empty():
            self.job_state = JobState(int(self.job_state) + 1)
            for pl in ScanProbe.gen_scan_payloads(self):
    ...
    def _security_note(self, body, k=''):
        with self.output_lock:
            self.build_html.add_list("note", body, k, self.url)
            logger_result.info(body + ":" + self.url)

    def _security_info(self, body, k=''):
        with self.output_lock:
            self.build_html.add_list("info", body, k, self.url)
            logger_result.info(body + ":" + self.url)

    def _security_warning(self, body, k=''):
        with self.output_lock:
            self.build_html.add_list("warning", body, k, self.url)
            logger_result.warning(body + ":" + self.url)

    def _security_hole(self, body, k=''):
        with self.output_lock:
            self.build_html.add_list("hole", body, k, self.url)
...
class BuildHtml(object):
    ...
    def add_list(self, level, value, k='', domain=''):
        ...
        if level not in self.dict[domain]:
            raise Exception("Building error:level not in dict")
    ...
    def mutiBuild(self):
        ...
        w9scan_html = w9scan_html.replace("{{scantime}}", "default time")
        ...
        try:
            ...
            raise Exception("Building result faild!")

        w9scan_html = w9scan_html.replace("{{content}}", ' '.join(full))
        filename = os.path.join(GlobalConfig.RESULT_FOLDER, "BatchScanning" + "_" + str(int(time.time())) + ".html")
        result = open(filename, "w")
        result.write(w9scan_html)
        result.close()
        logging.info("success saved :" + filename)
    ...
    def build(self):
        ...
        # build
        DomainRoot = ''.join(self.dict.keys())
        ...
        try:
            w9scan_html = base64.b64decode(w9scan_html)
            w9scan_html = w9scan_html.replace("{{url}}", DomainRoot )
            w9scan_html = w9scan_html.replace("{{scan_all_port}}", "default")
            w9scan_html = w9scan_html.replace("{{ThreadNum}}", "default")
            w9scan_html = w9scan_html.replace("{{select_plugin}}", "default")
            w9scan_html = w9scan_html.replace("{{reportTime}}", str(reportTime))
            w9scan_html = w9scan_html.replace("{{scantime}}", "default")
        except Exception:
            raise Exception("BuildHtml Error Exception")
        ...
        try:
            for url, content in self.dict.items():
                ...
                for key, value in content.items():
                    try:
                        ...
                        raise Exception("Save Report Exception")
            ...
            filename.replace(":", "_")
            filename = os.path.join(GlobalConfig.RESULT_FOLDER, filename)
            result = open(filename, "w")
            result.write(w9scan_html)
            result.close()
            logging.info("success saved :" + filename)
            ...
            raise Exception("Sava Faild! error:" + traceback.format_exc())
