...
def isExist(url):
    ...
    # 本来有 code ==200 的 但是测试自己搭建的其他站点的时候  返回的是405 ，所以不判断状态码了
    if 'XML-RPC server accepts POST requests only' in res:
        return True
    return False
...
def getWeakList():
    topuser2 = ['root', 'wordpress', 'www', 'test', 'admin', 'system', 'sys', 'sysadm', 'sysadmin', 'manager', 'scmadmin', 'super', 'superuser', 'superadmin', 'superman', 'smc', 'webadmin', 'websecadm', 'wlse', 'wlseuser', 'wradmin', 'Guest', 'naadmin',
                'netadmin', 'netman', 'adm', 'admin', 'admin2', 'administrator', 'adminstat', 'adminstrator', 'adminttd', 'adminuser', 'adminview', 'anonymous', 'Admin', 'Administrator', 'Alphanetworks', 'Anonymous', 'Any', 'ADMINISTRATOR', 'ADMN', 'security', 'mail']
